...
typedef struct PatikaCommand {
    ...
    union {
        ...
        struct { AgentID agent_id; } remove_agent;

        /** @brief Payload for @ref CMD_SET_GOAL . */
        struct { AgentID agent_id; int32_t goal_q, goal_r; } set_goal;

        /** @brief Payload for @ref CMD_BIND_BARRACK . */
        struct { AgentID agent_id; BarrackID barrack_id; } bind_barrack;

        /** @brief Payload for @ref CMD_SET_TILE_STATE . */
        struct { int32_t q, r; uint8_t state; } set_tile;

        /** @brief Payload for @ref CMD_COMPUTE_NEXT . */
        struct { AgentID agent_id; } compute_next;

        /** @brief Payload for @ref CMD_BATCH_COMPUTE_NEXT (reserved). */
        struct { uint32_t dummy; } batch_compute;
        ...
        struct {
            ...
            BarrackID* out_barrack_id; /**< Optional: set on processing; must remain valid until after @ref patika_tick . */
        } add_barrack;
    };
} PatikaCommand;
